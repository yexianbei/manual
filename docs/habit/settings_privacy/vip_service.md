...
1. 定义与适用：锁机功能指用户在设定的时间段内限制设备使用以提升专注；惩罚支付指用户在锁机期间提前解锁、绕过或削弱限制时触发的支付金额。

2. 权益排除：惩罚支付不属于任何会员权益范围，包括但不限于永久会员；任何会员等级均不享有惩罚支付的减免、豁免或抵扣。

3. 收费依据：惩罚支付的费率、计费方式（按次、按时长或按额度）以产品内实时展示为准；如页面另有明确规则的，从其规定。

4. 触发条件：用户在锁机期间发生以下任一行为时即时触发惩罚支付：
...
5. 支付与扣款：用户绑定支付方式后，触发时自动扣款；余额不足时系统将提示并可能暂停相关功能，但用户仍负有补足支付义务。

6. 目的与性质：惩罚支付用于行为约束，旨在帮助减少不必要的手机使用、提升专注，不属于商品或服务的对价，不构成行政意义上的罚款或处罚，不属于会员费。

7. 使用即同意：用户启用或实际使用锁机功能、设置惩罚支付选项、或在锁机期间发生触发行为，即视为已阅读、理解并同意本条款及相关规则。

8. 退费与异常：已触发的惩罚支付不予退还；因系统故障导致的误扣款，用户可在7日内提交申诉并提供账号信息、锁机时间段、设备与系统信息等，经核实属实的将予以全额返还。

9. 规则变更：平台可基于运营需要变更惩罚支付规则、费率或触发条件，并通过产品内公告或页面更新告知；变更生效后用户继续使用视为同意该等变更。

10. 特别人群：未成年人使用锁机功能需经其监护人同意并合理设置额度与频率，监护人承担相应监督责任。

11. 法律与争议：本条款为会员服务协议的组成部分，与主协议具有同等效力；如与主协议存在不一致，以本条款就锁机功能惩罚支付事项的约定为准。争议应先行通过客服渠道协商解决，协商不成的由平台注册地有管辖权的人民法院裁决。

12. 生效条款：本条款自平台公布之日起生效。
