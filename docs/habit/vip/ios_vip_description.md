...
- 小习惯按年订阅提供 7 天免费试用。根据 Apple 的结算规则，系统可能会在试用期结束前（通常不超过 24 小时）发起扣费，这属于正常行为；免费试用天数依然有效，会员时效将从试用期结束后开始计算。如不想续订，请在试用结束前在 iPhone 的订阅管理中取消订阅，具体操作可在[「取消订阅」](/habit/vip/ios_cancel_subscription)中查看；
...
3. ### 为什么点了「7天免费试用后」直接扣了款？
   ...
   根据 Apple 的设定，每位用户（以 Apple ID 作为唯一身份标识）在同一产品内通常仅可使用一次试用权限；若您已使用过试用，再次点击会直接进入付费扣款。若您是首次试用但出现“提前扣费”，一般是 Apple 可能会在试用期结束前（通常不超过 24 小时）发起扣费的正常流程；免费试用天数依然有效，会员有效期会在试用结束后才开始计算。
